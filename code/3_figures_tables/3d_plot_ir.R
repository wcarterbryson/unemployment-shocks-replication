## File description:
## Plot Figure 3 and 4: impulse response functions
...
plot_ir <- function(mdl, in_dir, out_dir,
    ...
    ## Create plot
    pdf(paste0(out_dir, "/", mdl, "/", irf_name, ".pdf"),
...
for (mdl in mdls) {
    for (ee in 1:N) {
        for (vv in 1:N) {
            plot_ir(mdl, ores_dir, ofig_dir, vv, ee, Nhp,
